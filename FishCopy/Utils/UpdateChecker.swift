...
class UpdateChecker {
                    ...
                    if let assets = json["assets"] as? [[String: Any]] {
                        ...
                        DispatchQueue.main.async {
                            if shouldUpdate {
                                ...
                                // 已经是最新版本
                                if !silent {
                                    self.showUpdateAlert(title: "已是最新版本", message: "您当前使用的 FishCopy（版本 \(self.currentVersion)）已是最新版本。")
                                }
    ...
    private func downloadUpdate(from url: URL) {
        ...
        downloadTask = session.downloadTask(with: url) { (tempURL, response, error) in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    notification.informativeText = "FishCopy更新已下载完成，点击安装"
        ...
        notification.informativeText = "FishCopy正在下载最新版本，下载完成后将通知您"
        notification.soundName = NSUserNotificationDefaultSoundName
        NSUserNotificationCenter.default.deliver(notification)
    }
} 